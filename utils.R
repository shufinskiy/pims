...
players_on_court <- function(df, order_fill = TRUE) {
  ...
  if(inherits(df$PCTIMESTRING, "character")){
    ...
    df <- dplyr::mutate(df, PCTIMESTRING = convert_time_to_second(df, PCTIMESTRING))
  }
  
  l <- lapply(sort(unique(df$PERIOD)), function(x){
    
    df_period <- dplyr::filter(df, PERIOD == x)
...
convert_time_to_second <- function(data, column){
  ...
  data %>%
    tidyr::separate(!!column, c('MIN', 'SEC'), sep = ':') %>%
    dplyr::mutate_at(c('MIN', 'SEC'), as.numeric) %>%
    dplyr::mutate(!!column := ifelse(PERIOD < 5, abs((MIN * 60 + SEC) - 720 * PERIOD), abs((MIN * 60 + SEC) - (2880 + 300 * (PERIOD - 4))))) %>%
    dplyr::select(!!column) %>%
    dplyr::pull()
}