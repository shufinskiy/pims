#### Скрипт выполнения meta_pims (расчёт pims с доп информацией)
...
transform_shots <- shots[LOC_Y <= 297.5, .(GAME_ID, GAME_EVENT_ID, PLAYER_ID, TEAM_ID, SHOT_TYPE, LOC_X, LOC_Y, SHOT_ATTEMPTED_FLAG, SHOT_MADE_FLAG)]
...
name_min_group <- if (min_group == cnt_shots_on) 'on' else 'off'